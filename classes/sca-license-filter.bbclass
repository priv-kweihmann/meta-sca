...
def sca_get_module_licenses(d):
    return d.getVar("LICENSE", True).replace(" ", "").split("&")

def sca_is_wildcard_lic(d):
    return any([x for x in d.getVar("SCA_AUTO_LICENSE_FILTER", True).split(" ") if x == ".*"])

def sca_license_filter_match(d, pkglic=[]):
    import re
    modlics = pkglic or sca_get_module_licenses(d)
    for item in d.getVar("SCA_AUTO_LICENSE_FILTER", True).split(" "):
        for lic in modlics:
...
def sca_filter_by_license_image(d):
    ...
    if not d.getVar("IMAGE_ROOTFS", True) or not os.path.exists(d.getVar("SCA_IMAGE_PKG_LIST", True)):
...
def sca_filter_by_license(d):
    if d.getVar("SCA_MODE", True) == "image":
