...
python do_sca_darglint() {
    import os
    import subprocess
    d.setVar("SCA_EXTRA_SUPPRESS", d.getVar("SCA_DARGLINT_EXTRA_SUPPRESS", True))
    d.setVar("SCA_EXTRA_FATAL", d.getVar("SCA_DARGLINT_EXTRA_FATAL", True))
    d.setVar("SCA_SUPRESS_FILE", os.path.join(d.getVar("STAGING_DATADIR_NATIVE", True), "darglint-{}-suppress".format(d.getVar("SCA_MODE", True))))
    d.setVar("SCA_FATAL_FILE", os.path.join(d.getVar("STAGING_DATADIR_NATIVE", True), "darglint-{}-fatal".format(d.getVar("SCA_MODE", True))))

    tmp_result = os.path.join(d.getVar("T", True), "sca_raw_darglint.txt")
    d.setVar("SCA_RAW_RESULT_FILE", tmp_result)
    cmd_output = ""

    os.makedirs(os.path.join(d.getVar("T", True), "darglintout"), exist_ok=True)
    ## Run
    ...
    ## Create data model
    d.setVar("SCA_DATAMODEL_STORAGE", "{}/darglint.dm".format(d.getVar("T", True)))
