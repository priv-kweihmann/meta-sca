...
def create_inventory(d, target_path):
    ...
    for item in clean_split(d, "_SCA_ANSIBLE_GLOBAL_VARS"):
        inv["all"]["vars"][item] = d.getVar("SCA_ANSIBLE_PATH_{}".format(item), True)
...
python do_sca_ansible() {
    ...
    d.setVar("SCA_SUPRESS_FILE", os.path.join(d.getVar("STAGING_DATADIR_NATIVE", True), "ansible-{}-suppress".format(d.getVar("SCA_MODE", True))))
    d.setVar("SCA_FATAL_FILE", os.path.join(d.getVar("STAGING_DATADIR_NATIVE", True), "ansible-{}-fatal".format(d.getVar("SCA_MODE", True))))
    ...
    for pb_glob in clean_split(d, "SCA_ANSIBLE_PLAYBOOKS"):
        for playbook in glob.glob(os.path.join(d.getVar("STAGING_DATADIR_NATIVE", True), "ansible_sec") + "/" + pb_glob):
    ...
    ## Create data model
    d.setVar("SCA_DATAMODEL_STORAGE", "{}/ansible.dm".format(d.getVar("T", True)))
