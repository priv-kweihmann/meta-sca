...
def do_sca_configcheck_run_postfix(d):
    return ["/bin/sh", "-c", "[ ! -z $(which postfix) ] && postfix -c {} check".format(d.getVar("SCA_CONFIGCHECK_postfix_CONFIGDIR", True))]
    ...
    for m in re.finditer(pattern, toolout, re.MULTILINE):
        try:
            g = sca_get_model_class(d,
                                    ...
                                    Tool="configcheck",
                                    BuildPath=buildpath,
                                    File=m.group("file") or d.getVar("SCA_CONFIGCHECK_postfix_CONFIGDIR", True),
