...
python do_sca_pyfindinjection_core() {
    import os
    import subprocess
    d.setVar("SCA_EXTRA_SUPPRESS", d.getVar("SCA_PYFINDINJECTION_EXTRA_SUPPRESS", True))
    d.setVar("SCA_EXTRA_FATAL", d.getVar("SCA_PYFINDINJECTION_EXTRA_FATAL", True))
    d.setVar("SCA_SUPRESS_FILE", os.path.join(d.getVar("STAGING_DATADIR_NATIVE", True), "pyfindinjection-{}-suppress".format(d.getVar("SCA_MODE", True))))
    d.setVar("SCA_FATAL_FILE", os.path.join(d.getVar("STAGING_DATADIR_NATIVE", True), "pyfindinjection-{}-fatal".format(d.getVar("SCA_MODE", True))))

    _args = [os.path.join(d.getVar("STAGING_BINDIR_NATIVE", True), "python-native", "python")]
    _args += [os.path.join(d.getVar("STAGING_BINDIR_NATIVE", True), "py-find-injection")]

    result_raw_file = os.path.join(d.getVar("T", True), "sca_raw_pyfindinjection.txt")
    ...
    ## Create data model
    d.setVar("SCA_DATAMODEL_STORAGE", "{}/pyfindinjection.dm".format(d.getVar("T", True)))
