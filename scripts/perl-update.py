...
def get_hashes(args, url):
    try:
        ...
        failure_log(
            args.faillog, "Can't generate hashes for URL:{}".format(url))
...
def update_pkg(args):
    ...
    if not _pkg:
        failure_log(args.faillog, "Can't extract CPAN package name for {pkg}".format(
            pkg=args.recipename))
        print("Can't extract CPAN package name for {pkg}".format(
            pkg=args.recipename))
        return False
    ...
    for index, value in enumerate(_lines):
        ...
        _updated = True
    if not _updated:
        failure_log(args.faillog, "Version/package name {version} doesn't exist {pkg}".format(
            version=args.version, pkg=args.recipename))
        print("Version/package name {version} doesn't exist {pkg}".format(
            version=args.version, pkg=args.recipename))
        return False
