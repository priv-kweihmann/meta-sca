...
DEPENDS += "${@oe.utils.ifelse(float(d.getVar('PYTHON_BASEVERSION', True) or '0.0') >= 3.6, '${PYTHON_PN}-flake8-2020-native', '')}"
DEPENDS += "${@oe.utils.ifelse(float(d.getVar('PYTHON_BASEVERSION', True) or '0.0') >= 3.6, '${PYTHON_PN}-flake8-fixme-native', '')}"
DEPENDS += "${@oe.utils.ifelse(float(d.getVar('PYTHON_BASEVERSION', True) or '0.0') >= 3.6, '${PYTHON_PN}-flake8-strict-native', '')}"
DEPENDS += "${@oe.utils.ifelse(float(d.getVar('PYTHON_BASEVERSION', True) or '0.0') >= 3.6, '${PYTHON_PN}-flake8-variables-names-native', '')}"
